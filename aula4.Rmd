...
```{r}
dentus_ancestral = data.frame(species =  c("root", "ABCD", "DB", "AC"), dentus_ancestral)
```
...
Agora podemos usar esse vetores de caracteres para calcular nossos vetores de mudança fenotípica ao longo da filogenia, os famigerados $\Delta z$. Para isso, é só subtrair o descendente do ancestral. Por exemplo, para o $\Delta z$ associado ao ramo que vai do nó AC para o terminal C, fazemos:
...
Este é o vetor de mudança evolutiva entre a espécie C e seu ancestral mais próximo.
...
Agora podemos usar essa função para calcular correlações entre todos os nossos vetores e responder perguntas evolutivas! Por exemplo, será que a diferenciação da população C se deu ao longo da sua linha de menor resistencia evolutiva?

```{r}
cov_matrices = dlply(dentus, .(species), function(x) cov(x[,1:4]))
PC1_C = eigen(cov_matrices$C)$vectors[,1]
abs(corVector(PC1_C, deltaZ_AC_C))
```
