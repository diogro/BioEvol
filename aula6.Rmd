...
Então, ficamos assim: correlação relativamente baixa entre $\Delta z$ e o primeiro componente. Isso  exclui a hipótese de restrição evolutiva. Mas, quando olhamos a correlação entre a seleção e a resposta, temos uma correlação de 0.90, indicando que a diversificação da espécie C se deu de forma alinhada com a seleção (e não seguindo a linha de menor resistência evolutiva). Será que o habito de vida dessa espécie pode trazer alguma pista?
