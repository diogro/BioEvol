...
Ou seja, diminuição da ulna e aumento de femur e tibia. Vamos agora tentar entender o que esse $\beta$ significa.
...
Então, ficamos assim: correlação relativamente baixa entre $\Delta z$ e o primeiro componente, indicando que a mudança evolutiva não foi na direção do componente alométrico. Quando olhamos a correlação entre a seleção e a resposta, temos uma correlação de 0.90, indicando que a diversificação da espécie C se deu de forma alinhada com a seleção (e não seguindo a linha de menor resistência evolutiva). Será que o habito de vida dessa espécie pode trazer alguma pista?
